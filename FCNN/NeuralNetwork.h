...
#define MAX2(a, b)							((a) > (b) ? (a) : (b)) // ReLU 

typedef double D;  // float 으로 사용해도 가능

class NeuralNetwork
{
public:
    int num_input_;  // input layer의 Neuron 개수
    int num_output_; // output layer의 Neuron 개수
    int num_all_layers_; // num_all_layers_ = num_hidden_layers_ + 2(input + output)
